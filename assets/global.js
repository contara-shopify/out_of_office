...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    if (!this.currentVariant) {
      ...
      this.updateShareUrl();
      this.filterMedia(); 
      this.afterMediaIconsChange();
      this.subheadingChange();
      this.tabContentChange();
    }
  }


  tabContentChange(){
    let get_variant_id = this.currentVariant.id;
    //Change product tabs base on variant
          let productTab_selector = document.getElementById('product_tabs_'+get_variant_id);
          let productTab = productTab_selector.innerHTML;
          $('.product__ddiln_waper').empty().html(productTab);
  }

  subheadingChange(){
    let get_variant_id = this.currentVariant.id;
    //Change product subtitle base on variant
          let productSubitle_selector = document.getElementById('product_subtitle_'+get_variant_id);
          let productSubitle = productSubitle_selector.innerHTML;
          $('.sub-title').empty().html(productSubitle);
  }

  afterMediaIconsChange(){
    let get_variant_id = this.currentVariant.id;
    //After media icons #1
          ...
          //console.log("icon 1: "+productAfterMediaIcons_1_html);
          //console.log("icon 1 ID: #product_after_media_icon_1_"+get_variant_id);
   ...
   filterMedia() {
      ...
      $('[thumbnail-filter]').hide();

      let selected_variant = this.currentVariant.title.replace(/\s*\/\s*/g, "-").replace(/ /g, "-").toLowerCase();
    
      //let selected_variant = this.currentVariant.featured_media.alt;
      
      let selected_attribute = '[thumbnail-filter~="' + selected_variant + '"]';
